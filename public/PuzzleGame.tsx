...
export default function PuzzleGame(props: PuzzleGameProps) {
    ...
    const cardHeight = useMemo(
        () =>
            clamp(
                isPortraitContainer
                    ? Math.round(cardWidth * 0.88)
                    : Math.min(340, Math.round(cardWidth * 0.68)),
                240,
                Math.max(240, size.height - minSideMargin * 2)
            ),
        [cardWidth, isPortraitContainer, minSideMargin, size.height]
    )
    const infoPanelWidthResolved = cardWidth

    // Horizontally center the panel in the container/screen, clamped inside side margins.
    const infoPanelLeft = useMemo(
        () =>
            clamp(
                (size.width - cardWidth) / 2,
                minSideMargin,
                Math.max(minSideMargin, size.width - minSideMargin - cardWidth)
            ),
        [cardWidth, minSideMargin, size.width]
    )
    // Vertically center the panel in the container/screen, clamped inside top/bottom margins.
    const infoPanelTop = useMemo(
        () =>
            clamp(
                Math.round((size.height - cardHeight) / 2),
                minSideMargin,
                Math.max(minSideMargin, size.height - minSideMargin - cardHeight)
            ),
        [cardHeight, minSideMargin, size.height]
    )
